...
// error in destroy the charts that already exists, if any item clicked after using inputs to filter the data, there is an error I cannot find any solution for it. if you know how to fix, please send the answer to my email.
// sol 1: remove click in initialization, add it only after inputs.
// sol2 : create new html file information.
...
$(document).ready(function () {
        ...
        displayObjects(group) {
            ...
            for (const customer of result) {
                tableBox += `
                    <tr class="rowItem">
                        <td>
                            <a target="_blank" href="info.html?id=${customer.customer_id}&name=${encodeURIComponent(customer.customer_name)}" 
                               class="customerName text-capitalize" data_id='${customer.customer_id}'>
    ...
    class Filters extends displayData {
        ...
        applyFilters() {
            let filteredData = this.final.filter(customer =>
                customer.customer_name.toLowerCase().startsWith(this.nameFilter) &&
                customer.total_amount >= this.amountFilter
            );

            let tableBox = '';
            let matchFound = false;
            for (let i = 0; i < filteredData.length; i++) {
                let customer = filteredData[i];
                tableBox += `
                    <tr class="rowItem">
                        <td>
                            <a target="_blank" href="info.html?id=${customer.customer_id}&name=${encodeURIComponent(customer.customer_name)}" 
                        ...
                        <td class="customerAmount">${customer.total_amount}</td>
                    </tr>`;
                matchFound = true;
            }
            ...
            document.getElementById('rowData').innerHTML = tableBox;
            return filteredData;
        }
    }
    ...
    class assignEvents {
        ...
        async initialize() {
            ...
            this.filterHandler = new Filters(final, this.dataHandler, this.chartHandler);
...
//         applyFilters() {
//             let filteredData = this.finalData.filter(customer => 
//                 customer.customer_name.toLowerCase().startsWith(this.nameFilter) &&
//                 customer.total_amount >= this.amountFilter
//             );
...
//                     <tr class="rowItem">
//                         <td>
//                             <a href="info.html?id=${customer.customer_id}&name=${encodeURIComponent(customer.customer_name)}" 
//                                class="customerName text-capitalize" data_id='${customer.customer_id}'>
//                                ${customer.customer_name}
//                             </a>
//                         </td>
